...
const [DATA_INIT,DATA_INIT_SUCCESS,DATA_INIT_FAILURE] = createRequestActionTypes(
    'sleepData/DATA_INIT'
)

const [EXISTS,EXISTS_SUCCESS,EXISTS_FAILURE] = createRequestActionTypes(
    'sleepData/EXISTS',
)

const [MERGE_DATA,MERGE_DATA_SUCCESS,MERGE_DATA_FAILURE] = createRequestActionTypes(
    'sleepData/MERGE_DATA',
)

const [SET_START_SLEEP,SET_START_SLEEP_SUCCESS,SET_START_SLEEP_FAILURE] = createRequestActionTypes(
    'sleepData/SET_START_SLEEP',
)

const [SET_FINISH_SLEEP,SET_FINISH_SLEEP_SUCCESS,SET_FINISH_SLEEP_FAILURE] = createRequestActionTypes(
    'sleepData/SET_FINISH_SLEEP',
)

const [READ_WEEK,READ_WEEK_SUCCESS,READ_WEAK_FAILURE] = createRequestActionTypes(
    'sleepData/READ_WEEK'
)
...
export const dataInit = createAction(DATA_INIT,({username,sleepDate}:any) => ({
    username,
    sleepDate,
}));
...
export const mergeData = createAction(MERGE_DATA,({username,sleepDate}:any) => ({
    username,
    sleepDate,
}))

export const setStartSleep = createAction(SET_START_SLEEP, ({username,sleepDate,startSleep}:any) => ({
    username,
    sleepDate,
    startSleep,
}));

export const setFinishSleep = createAction(SET_FINISH_SLEEP, ({username,sleepDate,finishSleep}:any) => ({
    username,
    sleepDate,
    finishSleep,
}));

export const readWeek = createAction(READ_WEEK,({username,sleepDate}:any) => ({
    username,
    sleepDate
}))
...
const setStartSleepSaga = createRequestSaga(SET_START_SLEEP,sleepDataAPI.setStartSleep);
const setFinishSleepSaga = createRequestSaga(SET_FINISH_SLEEP,sleepDataAPI.setFinishSleep);
...
const sleepData = createReducer(initialState, {
    ...
    [SET_FIELD]: (state:any, {payload: {form,year,month,day,hour,min}}: any) =>
        produce(state,(draft: { [x: string]: { [x: string]: any; }; }) => {
    ...
    [MERGE_DATA_SUCCESS]:(state:any,{payload:result}:any) => ({
        ...state,
        startSleep:{
            year: (result.startSleep.year ? result.startSleep.year : state.startSleep.year),
            month: (result.startSleep.month ? result.startSleep.month : state.startSleep.month),
            day: (result.startSleep.day ? result.startSleep.day : state.startSleep.day),
            hour: (result.startSleep.hour ? result.startSleep.hour : state.startSleep.hour),
            min: (result.startSleep.min ? result.startSleep.min : state.startSleep.min),
        },
        finishSleep:{
            year: (result.finishSleep.year ? result.finishSleep.year : state.finishSleep.year),
            month: (result.finishSleep.month ? result.finishSleep.month : state.finishSleep.month),
            day: (result.finishSleep.day ? result.finishSleep.day : state.finishSleep.day),
            hour: (result.finishSleep.hour ? result.finishSleep.hour : state.finishSleep.hour),
            min: (result.finishSleep.min ? result.finishSleep.min : state.finishSleep.min),
        },
    }),
